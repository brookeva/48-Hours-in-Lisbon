  ...
  */
  ...
  updateDataLayer(data) {
    ...
    const geoJsonLayer = L.geoJSON(data, { pointToLayer: (p, latlng) => L.marker(latlng, {icon: placeIcon})})
        .bindPopup((l) => l.feature.properties.Insta)
  ...
  async syncMapToSlide(slide) {
    ...
    const handleFlyEnd = () => {
      if (slide.showpopups) {
        layer.eachLayer((l) => {
          l.bindTooltip(l.feature.properties.Insta, { permanent: true });
          l.openTooltip();
        });
      }
  ...
  calcCurrentSlideIndex() {
    ...
    for (i = 0; i < this.slides.length; i++) {
      const slidePos = this.slides[i].offsetTop
     if (slidePos >= scrollPos - parentTop) {
        break;
     }
